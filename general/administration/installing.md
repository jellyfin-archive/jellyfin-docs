...
The default CentOS repositories don't carry FFmpeg, which the RPM requires. If you don't have FFmpeg installed, you can force the installation by using rpm instead of yum to install the RPM package from above:

```sh
sudo rpm -Uvh --nodeps ./jellyfin-xxxxxxx.rpm
```

Note that this defeats the purpose of yum's dependency management, and it may make it harder to update or uninstall Jellyfin in the future.

If you would like FFmpeg (which is required for video transcoding), you can install it from a number of private repositories, or you can build [Jellyfin's version](https://github.com/jellyfin/jellyfin-ffmpeg) on your own. This includes gathering the dependencies and compiling and installing them using the following commands (adapted from [the FFmpeg wiki](https://trac.ffmpeg.org/wiki/CompilationGuide/Centos)):

1. Download the build tools. This can be done using yum. Most can be uninstalled once you're done, besides `make`.
...
1. Create a directory to hold the FFmpeg source files. For these steps we'll assume it's `~/ffmpeg_sources`.

    ```sh
    mkdir ~/ffmpeg_sources
    ```

1. Install NASM, an assembler. Note that anywhere you see `make` (in any of the following steps), you can pass in a parameter to multithread the process, e.g. `make -j8` for 8 threads or `make -j10` for 10 threads. This may speed up your build time.
...
1. Install Yasm, another assembler.
...
1. Install libx264 for encoding H.264 video.
...
1. Install libx265 for encoding H.265 video.
...
1. Install libfdk_aac to encode AAC audio.
...
1. Install libmp3lame to encode MP3 audio.
...
1. Skip installing libopus, since the Opus audio codec [does not appear to be supported](https://jellyfin.org/docs/general/clients/codec-support.html).

1. Install libvpx to encode VP8/VP9 video.
...
1. Finally, install FFmpeg. It will be installed in `~/bin`.
...
1. Test your installation by running `ffmpeg` and `ffprobe`. Both should output some build information and help info. Also run `which ffmpeg` and copy down the location – you'll need it in a minute.

1. [Start Jellyfin for the first time](https://jellyfin.org/docs/general/quick-start.html) and get it set up. Then, on the web interface, go to the Dashboard. Under Server on the left side, click "Playback". Under "FFmpeg path", put the result of `which ffmpeg` from the previous step. Make sure all directories leading up to `ffmpeg` are traversable by the `jellyfin` user, i.e. that they have the `x` bit set in their permissions. Click save at the bottom.

You can test that transcoding works by playing a media file and choosing a quality with a high resolution and low bitrate. You can doubly verify this by opening the Playback Data dialog and ensuring that "Transcode Info" is shown rather than "Direct Stream Info". You will get an error if the file can't be transcoded.
