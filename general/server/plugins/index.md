...
* NotSupported - The plugin does not meet the minimum requirements for your version of Jellyfin. Check in there if another version is available, or contact the plugin developer.
* Malfunction - The plugin caused an error during that last startup, so it has been disabled. The error should be recorded in the system log.
